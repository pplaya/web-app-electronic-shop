...
const products = computed(() => productsStore.getProductsByCategory('Смартфоны').slice(0, 4))

const goToProduct = (product: any) => {
    router.push(`/catalog/phones`)
}
...
<template>
    <section class="mx-auto max-w-7xl px-6 py-12">
        <div class="flex justify-between items-center mb-5">
            <h2 class="text-2xl font-bold tracking-tight text-gray-900">Популярные телефоны</h2>
            <router-link to="/catalog/phones" class="text-cyan-600 hover:text-cyan-800 font-medium text-sm">
                Все телефоны
            </router-link>
        </div>
